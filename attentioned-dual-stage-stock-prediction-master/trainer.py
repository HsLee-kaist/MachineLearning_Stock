...
class Trainer:
    ...
    def train_minibatch(self, num_epochs, batch_size, interval):
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch + 1) % (interval) == 0 or epoch + 1 == num_epochs:
                torch.save(self.encoder.state_dict(), './models/encoder' + str(epoch + 1) + '-norm_50' + '.model')
                torch.save(self.decoder.state_dict(), './models/decoder' + str(epoch + 1) + '-norm_50' + '.model')
    ...
    def test(self, num_epochs, batch_size):
        ...
        plt.savefig('./results/res-' + str(num_epochs) +'-' + str(batch_size) +"_1" + '.png')
...
if __name__ == '__main__':
    ...
    trainer = Trainer(config.DRIVING, config.TARGET, 50, split, lr)
    if not test:
